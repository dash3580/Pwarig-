...
def run():
    ...
    for step in range(STEPS):
        ...
        if step % 100 == 0:
            ...
            np.save(f"npy_cycle/phi_A_{step:04d}.npy", phi_A)
            np.save(f"npy_cycle/phi_dot_A_{step:04d}.npy", phi_dot_A)
            np.save(f"npy_cycle/theta_A_{step:04d}.npy", theta_A)
            np.save(f"npy_cycle/theta_dot_A_{step:04d}.npy", theta_dot_A)
            np.save(f"npy_cycle/phi_B_{step:04d}.npy", phi_B)
            np.save(f"npy_cycle/phi_dot_B_{step:04d}.npy", phi_dot_B)
            np.save(f"npy_cycle/theta_B_{step:04d}.npy", theta_B)
            np.save(f"npy_cycle/theta_dot_B_{step:04d}.npy", theta_dot_B)
